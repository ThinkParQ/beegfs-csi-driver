...
# deploy the beegfs plugin driver.  This should be considered
...
# deploy beegfs plugin and registrar sidecar
echo "deploying beegfs components"
for i in $(ls ${BASE_DIR}/beegfs/*.yaml | sort); do
    echo "   $i"
...
while [ $(kubectl get pods 2>/dev/null | grep '^csi-beegfs.* Running ' | wc -l) -lt ${expected_running_pods} ]; do
    if [ $cnt -gt 30 ]; then
        echo "$(kubectl get pods 2>/dev/null | grep '^csi-beegfs.* Running ' | wc -l) running pods:"
        kubectl describe pods

        echo >&2 "ERROR: beegfs deployment not ready after over 5min"
        exit 1
    fi
    echo $(date +%H:%M:%S) "waiting for beegfs deployment to complete, attempt #$cnt"
    cnt=$(($cnt + 1))
    sleep 10
done
